...
- **折叠屏外屏（Galaxy Z Flip5，0.14.0-preview）**：合盖当 mini 服务器——外屏小组件看状态 / 启停 / 重启，
  外屏控制面板做全部壳侧设置，「完整界面」把 DeepCode 直接开在外屏上，见下节。

## 折叠屏外屏（Galaxy Z Flip）

面向「手机合盖常驻、尽量不开内屏」的用法。三块承载面，都不需要 root：

1. **外屏小组件（Flex Window）**：装好后到 **设置 → 外屏 → 小组件** 勾选「DeepCode 引擎（外屏）」，外屏左滑即见。
   卡片显示引擎状态（运行中 / 启动中 / 已停止 / 异常 / 运行时写入中）、延迟与运行时长、保活位（前台服务 / 电池白名单），
   四个按钮：**启动/停止**（按当前真实状态切换）、**重启**、**控制面板**、**完整界面**（后两者直接开在外屏上）。
   状态随看门狗每 5s 自动刷新，点卡片文字可手动刷新。
2. **外屏控制面板**（`CoverActivity`，为 720x748 设计）：状态卡 3s 采样；启停 / 重启 / 完整界面 / 控制台；
   保活与权限（电池优化白名单、所有文件访问、无障碍设备控制、通知权限、ADB 无线调试状态，点行直达系统页）；
   悬浮球 / 沉浸式状态栏 / 开发者日志开关；配置导出导入；检查更新。模型、供应商、密钥、插件等引擎侧设置点
   「完整界面」在同一块外屏打开 DeepCode 设置页（响应式 UI 在 360dp 宽即手机形态）。
3. **常驻通知动作**：引擎前台通知带「重启 / 停止 / 面板」三个按钮，外屏下拉通知面板即可操作。
